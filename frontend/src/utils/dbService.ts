...
export const getNoticias = async (): Promise<any[]> => {
  ...
  if (isMock) {
    ...
    const seed: any[] = []; // Semilla vacía por defecto
  ...
  const { data, error } = await supabase
    .from('noticias')
    .select('id, title, content, date, tag, image_url')
...
export const saveNoticia = async (noticiaForm: any, noticiaFile: File | null): Promise<void> => {
  ...
  if (noticiaFile) {
    finalImageUrl = await uploadFile(noticiaFile, 'noticias');
  }
  ...
  if (isMock) {
    const localData = localStorage.getItem('gibd_mock_noticias');
    const news = localData ? JSON.parse(localData) : [];
    ...
    localStorage.setItem('gibd_mock_noticias', JSON.stringify(news));
    return;
  }
