...
ruta_interna: "GIBD/BERT - GPT - Llama/Artículo CONAISI 2024/Notebooks/bge-m3_preguntas_similares_individualmente_gpt.ipynb"
tamanio_bytes: 111337
---

# Notebook: bge-m3_preguntas_similares_individualmente_gpt.ipynb

Ruta interna: `GIBD/BERT - GPT - Llama/Artículo CONAISI 2024/Notebooks/bge-m3_preguntas_similares_individualmente_gpt.ipynb`
...
**[Celda 2 - Código]**
```python
#Paquetes necesarios, con pip o conda según preferencia
'''
!pip install pandas
...
*Salida:*
```text
C:\Users\pablo\miniconda3\lib\site-packages\huggingface_hub\file_download.py:1132: FutureWarning: `resume_download` is deprecated and will be removed in version 1.0.0. Downloads always resume when possible. If you want to force a new download, use `force_download=True`.
  warnings.warn(
```

# Carga de la Reglamentación
...
# Generación u Obtención de Embeddings desde archivo npy


**[Celda 9 - Código]**
```python
embedding_file = 'articles_embeddings.npy'

# Verificar si el archivo ya existe
if os.path.exists(embedding_file):
    # Cargar los embeddings existentes
    loaded_embeddings = np.load(embedding_file, allow_pickle=True)
    articles_embedding = list(loaded_embeddings)
else:
    ...
    np.save(embedding_file, np.array(articles_embedding, dtype=object))

# Verificar que los embeddings se hayan cargado correctamente
print(f"Embeddings cargados o generados: {len(articles_embedding)} artículos.")
```


*Salida:*
```text
Embeddings cargados o generados: 674 artículos.
```
...
**[Celda 11 - Código]**
```python
def devolver_N_articulos_similares_preguntas(conj_preguntas, model, articles_embedding, articulos,N):

    question_embeddings=[]
    for pregunta in conj_preguntas:
        question_embeddings.append(model.encode(pregunta))
    ...
    for i, article_embedding in articles_embedding:
        ...
        for question in question_embeddings:
            distance = cosine_similarity(article_embedding.reshape((1, -1)), question.reshape((1, -1)))[0][0]
            sum=sum+distance
        results.append((articulos[i], sum))
...
## Importación de DataSet de preguntas etiquetadas
...
## Importado del dataset de consultas
...
for start in range(0, total_consultas, batch_size):
    ...
    for _, row in batch_df.iterrows():
        ...
        pregunta_consulta = row['Consulta_y']
        preguntas_similares = row[f'{n}_preguntas_similares'].split("\n")
        texto_consulta = row['Texto']

         # Obtener los N artículos más similares
        resultadoPrueba1= devolver_N_articulos_similares_preguntas(preguntas_similares, model, articles_embedding, articulos,total_consultas)
...
**[Celda 26 - Código]**
```python
#Opcional guardar resultados - Definir la ruta según la computadora a utilizar

csv_path = f'bge-m3_gpt4-similares_{n}_preguntas-individuales_combinadas-promedio.csv'
resultados_df.to_csv(csv_path, index=False)

```

## Cargar resultados para visualización
...
1-3: 94.0% (282 preguntas)
4-6: 2.7% (8 preguntas)
7-10: 1.7% (5 preguntas)
No aparece: 1.7% (5 preguntas)
```
    ...
    for posicion in range(1, 11):
        ...
        if not preguntas.empty:
            ...
            for _, row in preguntas.iterrows():
                ...
                '''
    print("\nDCG: ", nDCGTot/cantidad_acumulada)
