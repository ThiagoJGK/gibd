---
aliases: [2024-M3-Embedding_ Multi-Linguality, Multi-Functionality,...]
tags:
  - grupo/g3_texto
  - estado/archivo
  - tipo/documento
...
ruta_interna: "GIBD/BERT - GPT - Llama/Papers/2024-M3-Embedding_ Multi-Linguality, Multi-Functionality, Multi-Granularity  Text Embeddings Through Self-Knowledge Distillation.pdf"
tamanio_bytes: 794570
---

# 2024-M3-Embedding_ Multi-Linguality, Multi-Functionality, Multi-Granularity  Text Embeddings Through Self-Knowledge Distillation

Ruta interna: `GIBD/BERT - GPT - Llama/Papers/2024-M3-Embedding_ Multi-Linguality, Multi-Functionality, Multi-Granularity  Text Embeddings Through Self-Knowledge Distillation.pdf`
...
1. The model, code, and data is publicly available at
...
Doc-Level (≤8192)
Sentence-Level
Multi-LingualityMulti-FunctionalityMulti-Granularity
M3-Embedding
Multi-Lingual
Cross-Lingual
100+ Languages
...
arXiv:2402.03216v4  [cs.CL]  28 Jun 2024
linguality, which is able to support more than 100
...
of ensemble learning (B¨uhlmann, 2012), such het-
...
tion (Hofst¨atter et al., 2021; Ren et al., 2021; Zhang
...
2020). Another one is the curation of training and
...
trary language x, it is able to retrieve documentd in
language y from the corpusDy: dy ← fn∗(qx, Dy).
...
2020), CC-News (Hamborg et al., 2017) and the
...
IEE (Kim et al., 2023), PubMedQA (Jin et al.,
...
2021), T2-Ranking (Xie et al., 2023), LawGPT(Liu
et al., 2023), CMedQAv2 (Zhang et al., 2018), NLI-
zh2, and LeCaRDv2 (Li et al., 2023). For other
languages, we leverage the training data from Mr.
Tydi (Zhang et al., 2021b) and MIRACL (Zhang
et al., 2023c).
...
Pre-Tr ai ni ng1.2 Billion multilingual unsupervised dataFine-Tuni ng
•Labled data(EN, ZH, Mul)•Synthetic data (Long)
M3M3QueryAnswer
×
M3 M3QueryAnswer
×××dense score inter scoredensemulti-veclex
...
the embedding of passagep as ep = norm(Hp[0]).
...
(B¨uhlmann, 2012), the predictions from different
...
bels from another ranking model (Hofst¨atter et al.,
...
coder (an XLM-RoBERTa (Conneau et al., 2020)
...
dense retrieval methods: mDPR 3 (Zhang et al.,
2023b), mContriever 4 (Izacard et al., 2022),
...
with E5 mistral-7b, which leverages a much larger
...
BM25 mDPR mContriever mE5 large E5mistral-7b OpenAI-3 Dense Sparse Multi-vec Dense+Sparse All
...
the BEIR 6 (Thakur et al., 2021). Following the
previous study (Izacard et al., 2022), we report Re-
call@100 as the primary metric (Recall@20 is re-
ported as an auxiliary metric in the Appendix C.1).
For Dense+Sparse method and All method, we set
the same weights as in MIRACL dataset.
...
and mC4 (see Table 7), and NarrativeQA (Koˇcisk´y
et al., 2018; G ¨unther et al., 2023), which is
only for English. In addition to the previ-
ous baselines, we further introduce JinaEmbed-
dingv2 (G ¨unther et al., 2023), text-embedding-
...
their helpful feedback, and ACL 2024 and ACL
Rolling Review organizers for their efforts. This
research is supported by National Science and Tech-
nology Major Project (2023ZD0121504).
References
...
multilingual version of ms marco passage ranking
dataset. arXiv preprint arXiv:2108.13897.
Peter B ¨uhlmann. 2012. Bagging, Boosting and En-
semble Methods, pages 985–1022. Springer Berlin
...
mation Retrieval, SIGIR ’20, page 1533–1536, New
...
In Proceedings of the Workshop on Machine Reading
for Question Answering, pages 37–46, Melbourne,
...
balanced topic aware sampling. SIGIR ’21, page
113–122, New York, NY , USA. Association for Com-
puting Machinery.
...
2023. Coliee 2022 summary: Methods for legal doc-
ument retrieval and entailment. In New Frontiers in
Artificial Intelligence, pages 51–67, Cham. Springer
Nature Switzerland.
...
iao Ma, and Yiqun Liu. 2023. Lecardv2: A large-
...
and Beyond. Springer Nature.
Hongcheng Liu, Yusheng Liao, Yutong
Meng, and Yuhao Wang. 2023. Xiezhi:
Chinese law large language model.
...
Lv, Nan Duan, and Weizhu Chen. 2021a. Adver-
sarial retriever-ranker for dense text retrieval. arXiv
...
2021b. Mr. TyDi: A multi-lingual benchmark for
...
Jimmy Lin. 2023b. Toward best practices for training
...
Jimmy Lin. 2023c. MIRACL: A multilingual re-
...
2021), and mC4 (Raffel et al., 2020) datasets. We
