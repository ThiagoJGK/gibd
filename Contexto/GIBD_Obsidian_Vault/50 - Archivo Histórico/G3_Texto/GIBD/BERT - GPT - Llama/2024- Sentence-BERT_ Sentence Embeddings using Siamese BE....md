---
aliases: [2024- Sentence-BERT_ Sentence Embeddings using Siamese BE...]
tags:
  - grupo/g3_texto
  - estado/archivo
  - tipo/documento
...
ruta_interna: "GIBD/BERT - GPT - Llama/Papers/Referencias/2024- Sentence-BERT_ Sentence Embeddings using Siamese BERT-Networks.pdf"
tamanio_bytes: 548961
---

# 2024- Sentence-BERT_ Sentence Embeddings using Siamese BERT-Networks

Ruta interna: `GIBD/BERT - GPT - Llama/Papers/Referencias/2024- Sentence-BERT_ Sentence Embeddings using Siamese BERT-Networks.pdf`
...
would requireO(n2) score computations.
...
with the trainable weightWt∈ R3n×k:
o = softmax(Wt(u,v,|u−v|))
where n is the dimension of the sentence em-
beddings andk the number of labels. We optimize
...
sentencen, triplet loss tunes the network such that
the distance between a and p is smaller than the
distance between a and n. Mathematically, we
minimize the following loss function:
max(||sa−sp||−|| sa−sn|| +ϵ, 0)
withsx the sentence embedding fora/n/p,||·||
a distance metric and margin ϵ. Margin ϵ ensures
thatsp is at leastϵ closer tosa thansn. As metric
we use Euclidean distance and we setϵ = 1 in our
experiments.
...
Table 1: Spearman rank correlationρ between the cosine similarity of sentence representations and the gold labels
...
tionr to make the results comparable to Misra et
...
sentence embeddings u andv are used in combi-
...
signiﬁcantly worse thanMEAN or CLS-token strat-
...
Computational Semantics - V olume 1: Proceedings
of the Main Conference and the Shared Task, and
V olume 2: Proceedings of the Sixth International
...
sociation for Computational Linguistics (V olume 2:
...
ference on Computational Linguistics - V olume 1 ,
...
Computational Linguistics (ACL’04), Main V olume,
...
Linguistics: Human Language Technologies, V ol-
