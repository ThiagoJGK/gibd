---
aliases: [GIBD2025_ViT-Large_Busqueda-por-Similitud (ejec.lay-3-11....]
tags:
  - grupo/g4_imágenes
  - estado/archivo
  - tipo/notebook
...
ruta_interna: "GIBD/ViT/GIBD2025_ViT-Large_Busqueda-por-Similitud (ejec.lay-3-11.max.norm.blur.10k).ipynb"
tamanio_bytes: 34862315
---

# Notebook: GIBD2025_ViT-Large_Busqueda-por-Similitud (ejec.lay-3-11.max.norm.blur.10k).ipynb

Ruta interna: `GIBD/ViT/GIBD2025_ViT-Large_Busqueda-por-Similitud (ejec.lay-3-11.max.norm.blur.10k).ipynb`
...
def extract_embedding(image_path, mode, pooling, layers, device):
    ...
    if mode == "cls":  # CASO1
        ...
        embs = []
        for l in layers:
            feats = activations[l]                  # [1, tokens, dim]
            z = feats[:, 1:, :].max(dim=1).values   # max-pooling de patches
            z = F.normalize(z, dim=-1)              # normalizar por capa
            embs.append(z)

        emb = torch.stack(embs).mean(dim=0)         # promedio entre capas
        emb = F.normalize(emb, dim=-1)              # normalización final

    elif mode == "flat":  # CASO4
    # features = [1, 197, 1024]
    emb = features.reshape(features.size(0), -1)  # [1, 197*1024] -> []
    emb = F.normalize(emb, dim=-1)                # opcional normalización
